 ...
 * @author Amboara
 */
public class VoirStock extends HttpServlet {
...
//                              ArrayList ls = V_Stock.getListById(c, Integer.parseInt(id));
                                ArrayList ls = V_Stock.selectAll(c);

                              HttpSession session = request.getSession();
                              session.setAttribute("lsStock", ls);

//            RequestDispatcher rqd = request.getRequestDispatcher("associationMatiereStyle.jsp");
//            rqd.forward(request, response);
                              response.sendRedirect("listeStock.jsp");
                    } catch (Exception e) {
                              e.printStackTrace();
