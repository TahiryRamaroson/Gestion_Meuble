 ...
 * @author Amboara
 */
public class FormulaireFabrication extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        java.sql.Connection c = null;
                    try {
                              ...
                              ArrayList ls = V_MatiereStyle.getListById(c, Integer.parseInt(id));

                              HttpSession session = request.getSession();
                              session.setAttribute("lsMeub", lsMeubles);
                              session.setAttribute("lsVol", lsVolumes);
                              session.setAttribute("lsMatSt", ls);
                              session.setAttribute("style", id);
//            RequestDispatcher rqd = request.getRequestDispatcher("associationMatiereStyle.jsp");
//            rqd.forward(request, response);
                              response.sendRedirect("formulaireFabrication.jsp");
                    } catch (Exception e) {
                              e.printStackTrace();
